...
def makeBoxPlot(xColumn, yColumn, dataframe):
    '''Create box plot
    '''

    # Make plot
    fig = sns.boxplot(x=xColumn, y=yColumn, data=dataframe)

    # Clean column names for title and labels
    xLabel = xColumn.replace('_',' ').strip().title()
    yLabel = yColumn.replace('_',' ').strip().title()
